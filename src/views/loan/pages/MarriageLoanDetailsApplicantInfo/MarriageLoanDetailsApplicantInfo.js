...
const MarriageLoanDetailsApplicantInfo = () => {
  ...
  const getInfo = () => {
    ...
    getMarriageLoanDetailsInfo(body, "customer")
      .then((res) => {
        const convertList = [];
        ...
        dispatch(
          marriageLoan({
            ...
            applicantMarriageTryAgain: false,
          })
        );
      })
      .catch(() => {
        errorHandler(errorResponse);
        dispatch(marriageLoan({ applicantMarriageTryAgain: true }));
      });
  };
  const getImage = async (key) => {
    if (key) {
      ...
      dispatch(marriageLoan({ applicantDocumentInfo: arr }));
      let params;
      if (record) {
        params = {
          reference_number: record.reference_number,
      ...
      getMarriageLoanDetailsImages(params)
        .then((res) => {
          ...
          dispatch(
            marriageLoan({ applicantDocumentInfo: newArray, loading: false })
          );
        })
        .catch(() => {
          errorHandler(errorResponse);
          ...
          dispatch(marriageLoan({ applicantDocumentInfo: newArray2 }));
        });
    }
  };
      ...
      {applicantMarriageTryAgain ? (
        ...
        <div className={Classes["loan-wrapper"]}>
          <div className={Classes["detail-info-wrapper"]}>
            ...
            />
            <InfoBoxWithLabel
              name="sacrifice_quota"
              label={Dictionary.isar}
              value={
                applicantInfo?.sacrifice_quota
                  ? Dictionary.have
                  : Dictionary.notHave
              }
            />
...
export default MarriageLoanDetailsApplicantInfo;
