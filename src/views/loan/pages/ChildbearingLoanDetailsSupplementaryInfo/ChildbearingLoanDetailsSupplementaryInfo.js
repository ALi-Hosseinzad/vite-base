...
import {
  changeChildbearingLoanStatus,
  deleteChildbearingLoanFile,
  getChildbearingLoanDetailsImages,
  getChildbearingLoanDetailsInfo,
  getReactionSentences,
  uploadChildbearingLoanFile,
} from "helpers/APIFunction";

const ChildbearingLoanDetailsSupplementaryInfo = () => {
  const divRef = useRef();
  const docRef = useRef();
  ...
  const {
    ...
    supplementaryInfoChildbearingTryAgain,
  ...
  const getInfo = (key, blob, type) => {
    ...
    getChildbearingLoanDetailsInfo(body, "supplementary")
      .then((res) => {
        const convertList = [];
        res?.data?.document_info?.forEach((node, index) => {
          const convertObj = {
            ...
            viewImage: true,
            loadingUpload: false,
            canDelete: permissions.deleteDocumentChildBearing,
          };
      ...
      .catch(() => {
        ...
        dispatch(
          childbearingLoan({ supplementaryInfoChildbearingTryAgain: true })
        );
      });
  };
  ...
  useEffect(() => {
    if (uploadedFiles.length > 0) {
      ...
      dispatch(childbearingLoan({ supplementaryDocumentInfo: newArray }));
    }
  }, [uploadedFiles]);
  ...
  const onFinish = (values) => {
    if (
      ...
      if (body?.reference_number) {
        changeChildbearingLoanStatus(body)
          .then(() => {
            dispatch(
              setNotificationData({
      ...
      getReactionSentences({
        ...
        criteria: {
          ...
          criteria: [
            {
              key: "event",
              value: "CHILDBEARING_LOAN_REJECT_REASON",
              operation: "equals",
            },
          ],
        },
        ...
        .then((res) => {
          ...
          dispatch(childbearingLoan({ readyReactions: sentences }));
          setIcon(SearchIcon);
        })
        .catch(() => {
          errorHandler(errorResponse);
  ...
  const getImage = (key) => {
    ...
    dispatch(childbearingLoan({ supplementaryDocumentInfo: arr }));
    let params;
    if (record) {
      params = {
        reference_number: record.reference_number,
    ...
    getChildbearingLoanDetailsImages(params)
      ...
      .catch(() => {
        ...
        dispatch(childbearingLoan({ supplementaryDocumentInfo: newArray2 }));
        errorHandler(errorResponse);
      });
  };
    ...
    if (file && key && name) {
      ...
      if (value) {
        ...
        if (type === "pdf") {
          ...
          uploadChildbearingLoanFile(fileData, {
            description: name,
            ...
            .catch(() => {
              ...
              dispatch(
                childbearingLoan({ supplementaryDocumentInfo: newArray2 })
              );
            });
        } else {
          new Compressor(file, {
                ...
                if (result.size < 5000000) {
                  ...
                  uploadChildbearingLoanFile(fileData, {
                    ...
                    .catch(() => {
                      ...
                      dispatch(
                        childbearingLoan({
                          supplementaryDocumentInfo: newArray2,
                        })
                      );
                    });
                } else {
          ...
          <div className={Classes["details-info-card"]}>
            <div className={Classes["upload-doc-container"]}>
              <div>
                ...
                <p className={Classes["attention-2"]}>
                  توجه: نام مدارک ارسالی فقط شامل می‌تواند حروف فارسی و اعداد
                  باشد.
                </p>
              </div>
              {supplementaryDocumentInfo.length < 10 && (
                ...
                >
                  {Dictionary.add} {Dictionary.documentary} {Dictionary.new}
                </ButtonComponent>
              )}
            </div>
...
export default ChildbearingLoanDetailsSupplementaryInfo;
